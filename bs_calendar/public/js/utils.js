    ...
    if (frappe.user.has_role("HR User") || frappe.user.has_role("HR Manager")) {
      listview.page.add_inner_button(__('Allocate Monthly Leave (BS)'), async () => {
        ...
        const res = await frappe.call({
          method: "bs_calendar.custom_code.leave_allocation.monthly_leave_bs.get_bs_eligible_leave_types"
        });
        ...
        const d = new frappe.ui.Dialog({
          ...
          primary_action: values => {
            ...
            const proceed = () => {
              frappe.call({
                method: "bs_calendar.custom_code.leave_allocation.monthly_leave_bs.allocate_monthly_leave_bs",
                args: {
                  bs_year: bs.year,
                  bs_month: bs.month,
